...
class AdminGameControllerTest {
    ...
    void setUp() {
        ...
        lenient().when(gameController.toAdminResponse(any(Game.class))).thenAnswer(invocation -> {
            ...
            return new GameResponse(game.getId(), game.getSlug(), game.getTitleHu(), game.getDescriptionHu(), game.getImageUrl(), game.getTutorialHu(), game.getEmbedUrl(), game.isActive(), game.getSourceType().name(), 0L, String.valueOf(game.getReviewStatus()));
        });
    }
